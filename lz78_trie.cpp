...
std::pair<std::shared_ptr<LZ78::Dict::node>, int> LZ78::Dict::search(std::string& txt,
                                                                     int ini,
                                                                     std::shared_ptr<LZ78::Dict::node> curr) {
    ...
    for (int i = 0; i < curr->names.size();i++) {
        ...
        if (name == prefix) {
            ...
            return LZ78::Dict::search(txt, ini + prefix.length(), curr->refs[i]);
        }
    }
...
std::pair<int, int> LZ78::Dict::index(std::string& txt,
                                      std::shared_ptr<LZ78::Dict::node> root) {
    std::shared_ptr<LZ78::Dict::node> found;
    int end;
    std::tie(found, end) = LZ78::Dict::search(txt, 0, root);
...
void LZ78::Dict::add(std::string& w,
    ...
    std::shared_ptr<LZ78::Dict::node> found;
    int end;
    std::tie(found, end) = LZ78::Dict::search(w, 0, root);

    std::shared_ptr<LZ78::Dict::node> newNode(new LZ78::Dict::node);
...
std::string LZ78::encode(std::string& txt, std::string& ab) {
    ...
    std::vector<std::string> data_inv;
    std::shared_ptr<LZ78::Dict::node> root(new LZ78::Dict::node);
    data_inv.push_back("");
    ...
    while (i < n) {
        ...
        std::tie(j, l) = LZ78::Dict::index(sufix, root);
        ...
        LZ78::Dict::add(to_add, data_inv, root);
        i += l + 1;
    }
    return code;
}
    ...
    while (i < n) {
        ...
        while (1) {
            if (code.substr(i, 1) == ab.substr(0, 1)) {
                ...
                i+=1;
                aux = dic_entry + c;
                LZ78::Dict::add(aux, data_inv, root);
                // std::cout << LZ78::Dict::to_str(data_inv) << std::endl;
                break; 
            }
            w = code.substr(i, l+2);
